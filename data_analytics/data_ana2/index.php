...
$PAGE->set_url('/local/data_analytics/data_ana2/index.php'); // set the page URL
$PAGE->set_heading('Percentage of students who never log in from any class'); // set the page heading


$PAGE->set_pagelayout('standard');
$PAGE->navbar->add("Data Analytics", new moodle_url('/local/data_analytics/index.php'));
$PAGE->navbar->add("Percentage of students who never log in from any class");
        ...
        foreach ($categories as $category) {
            ...
            if ($category->id != $parent_id) { // Check if this is not the root category
                if (!$GLOBALS['DB']->count_records('course_categories', array('parent' => $category->id))) { // Check if this category has any children
                    ...
                    echo 'window.location.href = "' . $CFG->wwwroot . '/local/data_analytics/data_ana2/table.php?category_id=' . $category->id . '";';
                    echo '});';
                    echo '</script>';
                } else {
