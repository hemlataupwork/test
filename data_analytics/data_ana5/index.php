...
$PAGE->set_url('/local/data_analytics/data_ana5/index.php'); // set the page URL
$PAGE->set_heading('Percentage of students receiving less than 50% for an assessment'); // set the page heading
$PAGE->set_pagelayout('standard');
$PAGE->navbar->add("Data Analytics", new moodle_url('/local/data_analytics/index.php'));
$PAGE->navbar->add("Percentage of students receiving less than 50% for an assessment", new moodle_url('/local/data_analytics/index.php'));
        ...
        foreach ($categories as $category) {
            ...
            if ($category->id != $parent_id) { // Check if this is not the root category
                if (!$GLOBALS['DB']->count_records('course_categories', array('parent' => $category->id))) { // Check if this category has any children
                    ...
                    echo 'window.location.href = "' . $CFG->wwwroot . '/local/data_analytics/data_ana5/table.php?category_id=' . $category->id . '";';
                    echo '});';
                    echo '</script>';
                } else {
