...
$title = 'Percentage of students completing an assessment by the due date';
$pagetitle = $title;
$PAGE->set_title($title);
$PAGE->set_heading($title);
$context = context_system::instance();
$PAGE->set_context($context);
$PAGE->set_url('/local/data_analytics/data_ana4');
$PAGE->set_pagelayout('standard');
// $download = optional_param('download', '', PARAM_ALPHA);
$table = new test_table('uniqueid');
$table->is_downloading($download, 'Percentage of students completing an assessment by the due date', 'testing123');
if (!$table->is_downloading()) {
    // Only print headers if not asked to download data.
    // Print the page header.
    $PAGE->set_title('Percentage of students completing an assessment by the due date');
    $PAGE->set_heading('Percentage of students completing an assessment by the due date');
    $PAGE->navbar->add("Data Analytics", new moodle_url('/local/data_analytics/index.php'));

    $PAGE->navbar->add('Percentage of students completing an assessment by the due date', new moodle_url('/local/data_analytics/data_ana4/'));
...
$PAGE->navbar->add("$cat_name->name");
    ...
    $previewnode = $PAGE->navigation->add('Percentage of students completing an assessment by the due date', new moodle_url('/local/data_analytics/data_ana4/table.php'), navigation_node::TYPE_CONTAINER);
    echo $OUTPUT->header();
}
global $DB;
...
$fields = '@row_number := @row_number + 1 AS num,a.course, a.id , c.fullname,a.name,a.duedate ';
...
$table->define_baseurl("$CFG->wwwroot/local/data_analytics/data_ana4/table.php?category_id=$catid");
$table->out(10, true);
echo '
<style>
